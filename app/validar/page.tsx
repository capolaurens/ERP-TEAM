...
import { VALIDAR } from "@/lib/northdeco-validar";
import { POR_REHACER, revisionPorFile } from "@/lib/northdeco-estado";

export const metadata = {
  title: "Por validar · Northdeco — NAVYX",
...
/**
 * /validar: la MISMA tarjeta que /northdeco (visor, datos y foto de la tienda
 * al lado) pero con la lista de revisión interna (lib/northdeco-validar.ts):
 * lo que Lorenzo mira antes de decidir qué pasa a la galería del cliente.
 * Comparte gallery.js, el proxy del GLB y el feedback (checks y comentarios).
 */
export default async function ValidarPage() {
  ...
  const models = await leerGaleria(VALIDAR);
  ...
  return (
    <div className="nx">
      ...
      <header className="nx-head">
        ...
        <h1>Northdeco · piezas por validar</h1>
        <p className="nx-sub">
          Revisión interna antes de pasar a la galería del cliente: el modelo 3D
          con la foto de la tienda al lado. Gira cada pieza arrastrando; en el
          móvil, <b>Ver en tu espacio</b> la coloca con realidad aumentada.
        </p>
